...
import com.newsee.generator.provider.db.table.model.Column.EnumMetaDada;
/**
 * 
...
public class StringHelper {
   ...
   public static String defaultString(Object value) {
	...
	/**
	 * 将表单类型的数据type转换成字符型type
	 * @param type
	 * @return
	 * 0:text，1:select，2:date，3:radio,4:checkbox
	 */
	public static String getFormType(String type){
		if(isBlank(type)){
			return "";
		}
		if("0".equals(type)){
	...
	/**
	 * 将为1的string字符串转换成true，其他的转换成false
	 * @param booleanString
	 * @return
	 */
	public static boolean stringToBoolean(String booleanString){
		if(StringUtils.isNullOrEmpty(booleanString)){
			return false;
		}
		if("1".equals(booleanString) || "true".equals(booleanString.toLowerCase())){
			return true;
		}
		return false;
	}
